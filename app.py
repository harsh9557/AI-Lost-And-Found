            ...
            if not item_name or not location:
                ...
                if photo:
                    ...
                    photo_path = os.path.join(
                        "uploads",
                        "lost_" + str(uuid.uuid4()) + "_" + photo.name
                    )
        ...
        if submit:
            ...
            if not item_name or not location:
                ...
                if photo:
                    ...
                    photo_path = os.path.join(
                        "uploads",
                        "found_" + str(uuid.uuid4()) + "_" + photo.name
                    )
...
# =========================================
    ...
    st.caption(
        "AI score = 70% text similarity + 30% image similarity. "
        "Text considers item name, description, color and location."
    )
                ...
                if st.button(
                    ...
                    if not claim_message.strip():
                        ...
                        if created:
                            ...
                            st.warning(
                                "⚠️ You already have a pending claim for this item."
                            )
...
# =========================================
    ...
    with chart_col1:
        st.caption("Lost vs Found")
        st.bar_chart(
            {"Reports": {"Lost": lost, "Found": found}},
            height=300
        )

    with chart_col2:
        st.caption("Resolution")
        unresolved = max(reports - resolved, 0)
        st.bar_chart(
            {"Reports": {"Resolved": resolved, "Active": unresolved}},
            height=300
        )

    if reports > 0:
        resolution_rate = round((resolved / reports) * 100, 1)
